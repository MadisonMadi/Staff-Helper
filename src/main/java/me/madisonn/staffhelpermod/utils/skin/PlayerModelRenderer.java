...
public class PlayerModelRenderer {
    ...
    public static void renderPlayerModel(DrawContext context, String playerName, int x, int y, int size, float rotation, boolean showSecondLayer) {
        ...
        try {
            ...
            StaffHelperClient.LOGGER.error("Failed to render player model for: {}", playerName, e);
    ...
    private static UUID fetchUUIDFromAPI(String playerName) {
        try {
            ...
            StaffHelperClient.LOGGER.warn("Failed to fetch UUID for {}, using offline UUID", playerName, e);
            ...
            try {
                ...
                if (connection.getResponseCode() == 200) {
                    ...
                    if (json.has("id")) {
                        ...
                        StaffHelperClient.LOGGER.debug("Found UUID for {}: {}", playerName, uuid);
                        return uuid;
                    }
                }
                StaffHelperClient.LOGGER.debug("No Mojang profile found for: {}", playerName);
            } catch (Exception e) {
                StaffHelperClient.LOGGER.error("Mojang API error for {}", playerName, e);
            }
    ...
    private static void loadSkinFromMinecraftTextureServer(MinecraftClient client, String playerName, UUID uuid) {
        CompletableFuture.runAsync(() -> {
            try {
                ...
                StaffHelperClient.LOGGER.error("Error loading skin from texture server for {}", playerName, e);
            }
        });
    }
                ...
                if (property.has("name") && "textures".equals(property.get("name").getAsString())) {
                    ...
                    if (texturesJson.has("textures")) {
                        ...
                        if (textures.has("SKIN")) {
                            ...
                            StaffHelperClient.LOGGER.debug("Downloading skin for {} from: {}", playerName, skinUrl);
    ...
    private static void downloadAndRegisterSkin(MinecraftClient client, String playerName, String skinUrl) {
        CompletableFuture.supplyAsync(() -> downloadSkinImage(skinUrl))
                ...
                .exceptionally(throwable -> {
                    StaffHelperClient.LOGGER.error("Failed to download skin for {}", playerName, throwable);
                    return null;
                });
    }
    ...
    private static Optional<byte[]> downloadSkinImage(String skinUrl) {
        try {
            ...
            StaffHelperClient.LOGGER.error("Error downloading skin image", e);
        }
    ...
    private static void registerSkin(MinecraftClient client, String playerName, byte[] imageData) {
        client.execute(() -> {
            try (InputStream imageStream = new java.io.ByteArrayInputStream(imageData)) {
                ...
                StaffHelperClient.LOGGER.info("Successfully loaded skin for: {}", playerName);

                if (currentCache != null && currentCache.playerName().equals(playerName)) {
                    currentCache = null;
                }
            } catch (Exception e) {
                StaffHelperClient.LOGGER.error("Failed to register skin texture for {}", playerName, e);
            }
        });
    }
    ...
    public static void clearAllCache() {
        ...
        StaffHelperClient.LOGGER.info("Cleared all skin viewer cache");
    }
